...
class MIA:
    def __init__(
        ...
        self.posterior = torch.softmax(posterior, 1)
        # self.posterior = torch.exp(posterior)
        # self.posterior = torch.softmax(self.posterior, 1)
        ...
        for which_run in range(1, num_of_runs):
            ...
            if data_type == "Reddit":
                ...
                dataset = Planetoid(
                    root="./data/Coauthor_CS", name="Coauthor_CS", split="random"
                )

                num_train_Train_per_class = 1000
                num_train_Shadow_per_class = 1000
                num_test_Target = 15000
                num_test_Shadow = 15000

            elif data_type == "Coauthor_Phys":
                dataset = Planetoid(
                    root="./data/Coauthor_Phys", name="Coauthor_Phys", split="random"
                )

                num_train_Train_per_class = 1000
                num_train_Shadow_per_class = 1000
                num_test_Target = 5000
                num_test_Shadow = 5000
            elif data_type == "Coauthor_CS":
            ...
            if len(self.test_indices) > len(self.node_removes):
                self.test_indices, random_nodes = random.sample(
                    self.test_indices.tolist(), len(self.node_removes)
                ), random.sample(list(self.test_indices), 200)

            self.shadow_positive = self.posterior[self.train_indices[200:400]]
            ...
            # self.train_posterior = self.posterior[self.train_indices[:200]]
            ...
            def get_inductive_spilt(
                ...
                data = Data(
                    ...
                    shadow_train_idx=shadow_train_idx,
            ...
            # -----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*-----*
            # -------------------------------- TEST FUNCTION -----------------------------------------
            def test(model, isTarget=True, post=None):
                model.eval()
                ...
                if isTarget:
                    ...
                    pred = post[data_new.shadow_train_idx]
                    # pred_Intrain_ps = torch.exp(pred)
                    pred_Intrain_ps = pred  # torch.exp(pred)
                    ...
                    preds = post[data_new.shadow_test_mask]
                    # print("baba1")
                    ...
                    pred_out = preds.max(1)[1].to(device)
                    # pred_out_ps = torch.exp(preds)
                    pred_out_ps = preds  # torch.exp(preds)
            ...
            # pdb.set_trace()
            X = Propagation(data.x, data.edge_index).to(device)
            X_train = X[data_new.shadow_train_idx]
            y_train_ = F.one_hot(data.y.to(device)[data_new.shadow_train_idx]).float()
            y_train = data.y.to(device)[data_new.shadow_train_idx]
            weight = None
            # in our case weight should always be None
            assert weight is None
            # record the optimal gradient norm wrt the whole training set
            opt_grad_norm = 0
            b = 1e-2 * torch.randn(X_train.size(1), y_train_.size(1)).float().to(device)
            ...
            for epoch in range(1, model_training_epoch):
                if model_type == "SAGE":
                    ...
                    train_acc, test_acc, marco, micro = test(shadow_model, False, post)
                # print("approx train acc", approx_train_acc, "train_loss", train_loss)
            ...
            attack_train_data_loader = torch.utils.data.DataLoader(
                attack_train_data, batch_size=64, shuffle=True
            )
            ...
            attack_test_data_loader = torch.utils.data.DataLoader(
                attack_test_data, batch_size=64, shuffle=True
            )
            ...
            optimizer = torch.optim.Adam(
                attack_model.parameters(), lr=0.01, weight_decay=0.00001
            )  # 0.01 #0.00001
            ...
            self.attack_acc = test_accuracyInOut
            self.attack_auc = auroc
    ...
    def get_results(self):
        return self.acc_pos, self.f1_pos, self.attack_acc, self.attack_auc
