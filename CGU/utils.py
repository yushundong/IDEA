...
def lr_optimize(
    ...
    w = torch.autograd.Variable(
        torch.zeros(X.size(1)).float().to(device), requires_grad=True
    )
...
def ovr_lr_loss_entropy(w, X, y, lam, weight=None):
    """
     input:
        w: (d,c)
        X: (n,d)
    ...
    return:
        loss: scalar
    """
    z = batch_multiply(X, w)
    # print(F.cross_entropy(z, y).mean(0).sum() + lam * w.pow(2).sum() / 2)
    # pdb.set_trace()
    if weight is None:
        return F.cross_entropy(z, y).mean(0).sum() + lam * w.pow(2).sum() / 2
    else:
        return F.cross_entropy(z, y).mul_(weight).sum() + lam * w.pow(2).sum() / 2
...
def ovr_lr_eval(w, X, y):
    ...
    return:
        loss: scalar
    !!! note this is changed from the original code acc to f1
    """
    f1 = f1_score(
        y.to("cpu").numpy(),
        X.mm(w).max(1)[1].to("cpu").numpy(),
        average="macro",
    )
...
def ovr_lr_optimize(
    ...
    # zero initialization
    w = torch.autograd.Variable(
        torch.zeros(X.size(1), y.size(1)).float().to(device), requires_grad=True
    )
...
def ovr_lr_optimize_entropy(
    ...
    # zero initialization
    w = torch.autograd.Variable(
        torch.zeros(X.size(1), y_.size(1)).float().to(device), requires_grad=True
    )

    def closure():
        if b is None:
            return ovr_lr_loss_entropy(w, X, y, lam, weight)
        else:
            return ovr_lr_loss_entropy(w, X, y, lam, weight) + (b * w).sum() / X.size(0)
    ...
    for i in range(num_steps):
        optimizer.zero_grad()
        loss = ovr_lr_loss_entropy(w, X, y, lam, weight)
        if b is not None:
            if weight is None:
                loss += (b * w).sum() / X.size(0)
            else:
...
def get_propagation(
    ...
    """
    fill_value = 2.0 if improved else 1.0  # what does improved mean?
...
class MyGraphConv(MessagePassing):
    ...
    def forward(
        ...
        if isinstance(edge_index, Tensor):
            edge_index, edge_weight = get_propagation(  # yapf: disable prepare P
                edge_index,
                edge_weight,
    ...
    for i in range(num_classes):
        if int(C_size_exceed[i]) > 0:
            pick = np.random.choice(
                All_train_id[y_train == i], int(C_size_exceed[i]), replace=False
            )
...
def get_worst_Gbound_feature(lam, m, deg_m, gamma1=0.25, gamma2=0.25, c=1, c1=1):
    return (
        gamma2
        * ((2 * c * lam + (c * gamma1 + lam * c1) * deg_m) ** 2)
        / (lam**4)
        / (m - 1)
    )
...
def get_worst_Gbound_node(lam, m, K, deg_m, gamma1=0.25, gamma2=0.25, c=1, c1=1):
    return (
        gamma2
        * ((2 * c * lam + K * (c * gamma1 + lam * c1) * (2 * deg_m - 1)) ** 2)
        / (lam**4)
        / (m - 1)
    )
