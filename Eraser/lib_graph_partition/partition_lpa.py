...
class PartitionLPA(Partition):
    ...
    def partition(self):
        ...
        community_to_node = {
            key: c
            for key, c in zip(range(self.graph.number_of_nodes()), community_generator)
        }
        print(
            "Found %s communities by unconstrained LPA", len(community_to_node.keys())
        )
...
class PartitionConstrainedLPA(Partition):
    ...
    def partition(self):
        adj_array = nx.linalg.adjacency_matrix(self.graph).toarray().astype(bool)
        ...
        node_threshold = math.ceil(
            self.graph.number_of_nodes() / self.args["num_shards"]
        ...
        self.logger.info(
            " #. nodes: %s. LPA shard threshold: %s."
            % (self.graph.number_of_nodes(), node_threshold)
        )
        lpa = ConstrainedLPA(
            adj_array, self.num_shards, node_threshold, self.args["terminate_delta"]
        )
        ...
        pickle.dump(
            lpa_deltas,
            open(
                config.ANALYSIS_PATH + "partition/blpa_" + self.args["dataset_name"],
                "wb",
            ),
        )
