...
class DataStore:
    ...
    def determine_data_path(self):
        embedding_name = "_".join(
            (
                "embedding",
                self._extract_embedding_method(self.partition_method),
                str(self.args["ratio_deleted_edges"]),
            )
        )

        community_name = "_".join(
            (
                "community",
        ...
        self.train_test_split_file = (
            processed_data_prefix + "train_test_split" + str(self.args["test_ratio"])
        )
        ...
        self.unlearned_file = processed_data_prefix + "_".join(
            ("unlearned", str(self.args["num_unlearned_nodes"]))
        )

        self.target_model_file = (
            config.MODEL_PATH + self.dataset_name + "/" + target_model_name
        )
        self.optimal_weight_file = (
        ...
        dir_lists = [
            ...
            for s in [
                ...
                config.ANALYSIS_PATH + "removed_edges/",
            ]
        ]
        for dir in dir_lists:
            self._check_and_create_dirs(dir)

        self.removed_edges_file = (
            config.ANALYSIS_PATH
            + "removed_edges/"
            + self.dataset_name
            + "/"
    ...
    def load_raw_data(self):
        ...
        if self.dataset_name in ["cora", "pubmed", "citeseer"]:
            dataset = Planetoid(
                config.RAW_DATA_PATH, self.dataset_name, transform=T.NormalizeFeatures()
            )
            ...
            if self.dataset_name == "Coauthor_Phys":
                dataset = Coauthor(
                    config.RAW_DATA_PATH, name="Physics", pre_transform=self.transform
                )
                data = dataset[0]
                feature = data.x
                ...
                dataset = Coauthor(
                    config.RAW_DATA_PATH, name="CS", pre_transform=self.transform
                )
                data = dataset[0]
        else:
    ...
    def save_train_test_split(self, train_indices, test_indices):
        ...
        pickle.dump(
            (train_indices, test_indices), open(self.train_test_split_file, "wb")
        )
    ...
    def load_community_data(self, suffix=""):
        # self.logger.info(
        #     "loading community data from: %s" % (self.community_file + suffix)
        # )
            ...
            model_path = (
                "_".join(
                    (
                        ...
                        str(shard),
                        str(run),
                        str(self.args["num_unlearned_nodes"]),
                    )
                )
                + suffix
            )
    ...
    def save_posteriors(self, posteriors, run, suffix=""):
        ...
        # torch.save(posteriors, self.posteriors_file + "_" + str(run))

    def save_attack_posteriors(self, posteriors, run, suffix=""):
        torch.save(
            posteriors,
            self.posteriors_file
            + "_"
    ...
    def load_attack_posteriors(self, run, suffix=""):
        a = (
            self.args["exp"]
            if self.args["exp"] != "attack_unlearning"
            else "node_edge_unlearning"
        )
        # return torch.load(
        #     self.posteriors_file + "_" + str(run) + "_attack_" + a + "_" + suffix
        # )
    ...
    def save_removed_edges(self, removed_edges):
        # self.logger.info("saving removed edges")
        pickle.dump(removed_edges, open(self.removed_edges_file, "wb"))

    def load_removed_edges(self):
        # self.logger.info("loading removed edges")
        return pickle.load(open(self.removed_edges_file, "rb"))

    def save_is_exist(self, is_exist):
        """
        whether the removed edges exsit in this shared
        """
        # self.logger.info("saving is_exist")
        pickle.dump(is_exist, open(self.removed_edges_file + "_" + "is_exist", "wb"))

    def load_is_exist(self):
        # self.logger.info("loading is_exist")
        return pickle.load(open(self.removed_edges_file + "_" + "is_exist", "rb"))
