...
class NodeClassifier(GNNBase):
    ...
    def evaluate_model(self):
        ...
        if self.target_model == "GCN":
            out = self.model.inference(
                self.data.x, self.test_loader, self.edge_weight, self.device
            )
        else:
    ...
    def posterior(self):
        ...
        self._gen_test_loader()
        if self.target_model == "GCN":
            posteriors = self.model.inference(
                self.data.x, self.test_loader, self.edge_weight, self.device
            )
        else:
            posteriors = self.model.inference(
                self.data.x, self.test_loader, self.device
            )
    ...
    def _gen_train_loader(self):
        ...
        edge_index = utils.filter_edge_index(
            self.data.edge_index, train_indices, reindex=False
        )
    ...
    def _gen_test_loader(self):
        ...
        if not self.args["use_test_neighbors"]:
            edge_index = utils.filter_edge_index(
                self.data.edge_index, test_indices, reindex=False
            )
        else:
...
if __name__ == "__main__":
    ...
    train_indices, test_indices = train_test_split(
        np.arange((data.num_nodes)), test_size=0.2, random_state=100
    )
    data.train_mask, data.test_mask = torch.zeros(
