...
class ExpUnlearning(Exp):
    """
    especially for edge unlearning where the edge is deleted in partition
    ...
    def determine_target_model(self):
        ...
        if not self.args["is_use_batch"]:
            ...
            elif self.target_model_name == "MLP":
                self.target_model = MLP(num_feats, num_classes)
            elif self.target_model_name == "SGC":
                self.target_model = SGConvBatch(num_feats, num_classes)
            else:
    ...
    def aggregate(self, run):
        ...
        aggregator = Aggregator(
            run, self.target_model, self.data, self.shard_data, self.args
        )
