...
class ExpGraphPartition(Exp):
    ...
    def train_test_split(self):
        if self.args["is_split"]:
            ...
            (
                self.train_indices,
                self.test_indices,
            ) = self.data_store.load_train_test_split()
    ...
    def gen_train_graph(self):
        ...
        if self.args["ratio_deleted_edges"] != 0:
            self.args["edge_unlearning"] = True
            # self.logger.debug(
            #     "Before edge ratio deletion. train data  #.Nodes: %f, #.Edges: %f"
            ...
            removed_edges = np.array(
                [
                    x
                    for x in set(tuple(x) for x in original_edge_index.T.tolist())
                    - set(tuple(x) for x in self.data.edge_index.T.tolist())
                ]
            ).T
            print("Removed Edges:")

        elif self.args["num_unlearned_edges"] != 0:
            self.args["edge_unlearning"] = True
            # self.logger.debug(
            #     "Before edge num deletion. train data  #.Nodes: %f, #.Edges: %f"
            #     % (self.data.num_nodes, self.data.num_edges)
            # )
            ...
            self.data.edge_index = self._num_delete_edges(self.data.edge_index)
            ...
            self.args["edge_unlearning"] = False

        # save removed edges
        if self.args["edge_unlearning"]:
            self.data_store.save_removed_edges(removed_edges)
            self.data_store.load_removed_edges()
            self.removed_edges = removed_edges

        # decouple train test edges. train_edges only in the train graph, no test edges
        edge_index = self.data.edge_index.numpy()
        test_edge_indices = np.logical_or(
        ...
        # self.logger.debug(
        #     "After edge deletion. train graph  #.Nodes: %f, #.Edges: %f"
        #     % (self.train_graph.number_of_nodes(), self.train_graph.number_of_edges())
        # )
        # self.logger.debug(
        #     "After edge deletion. train data  #.Nodes: %f, #.Edges: %f"
        #     % (self.data.num_nodes, self.data.num_edges)
        # )
    ...
    def graph_partition(self):
        if self.args["is_partition"]:
            ...
            if self.args["edge_unlearning"]:
                is_exist = [0] * len(self.community_to_node)
                for i in np.unique(self.removed_edges):
                    for key, j in self.community_to_node.items():
                        if i in j:
                            is_exist[key] = 1
                self.data_store.save_is_exist(is_exist)

            # self.data_store.save_test_index(self.test_indices)
            # self.data_store.save_removed_edges
        else:
    ...
    def _prune_train_set(self):
        ...
        self.logger.debug(
            "Before Prune...  #. of Nodes: %f, #. of Edges: %f"
            % (self.train_graph.number_of_nodes(), self.train_graph.number_of_edges())
        )
        ...
        self.logger.debug(
            "After Prune... #. of Nodes: %f, #. of Edges: %f"
            % (self.train_graph.number_of_nodes(), self.train_graph.number_of_edges())
        )
    ...
    def _ratio_delete_edges(self, edge_index):
        ...
        unlearned_indices = np.setdiff1d(np.arange(edge_index.shape[1]), remain_indices)
        ##### save unlearned edges #####
        # np.save(
        #     unlearned_indices,
    ...
    def _num_delete_edges(self, edge_index):
        edge_index = edge_index.numpy()
        ...
        remain_indices = np.random.choice(
            unique_indices,
            int(unique_indices.shape[0] - self.args["num_unlearned_edges"]),
            replace=False,
        )
