...
parser.add_argument(
    "--prediction_path", type=str, default="data/pred/", help="prediction saved path"
)
parser.add_argument(
    "--partial_graph_path",
    type=str,
    ...
    for i in range(len(target_posterior_list)):
        for j in range(len(sim_metric_list)):
            ...
            target_sim = sim_metric_list[j](
                target_posterior_list[i][0], target_posterior_list[i][1]
            )
...
def write_auc(pred_prob_list, label, desc):
    ...
    with open("result/attack_0.txt", "a") as wf:
        for i in range(len(sim_list_str)):
            ...
            wf.write(
                "%s,%s,%d,%0.5f,%s\n"
                % (dataset, "attack0_%s_%s" % (desc, sim_list_str[i]), -1, i_auc, ratio)
            )
...
def process():
    ...
    test_path = partial_graph_path + "%s_train_ratio_%s_test.json" % (dataset, "0.9")
    ...
    for row in test_data:
        ...
        reference_posterior_list.append([row["dense_pred0"], row["dense_pred1"]])
