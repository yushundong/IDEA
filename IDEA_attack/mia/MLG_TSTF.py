...
class MIA:
    def __init__(
        ...
        for which_run in range(1, num_of_runs):
            ...
            elif data_type == "Coauthor_CS" or data_type == "CS":
                dataset = Coauthor("./data/Coauthor_CS", name="CS")

                num_train_Train_per_class = 10
                num_train_Shadow_per_class = 400
                num_test_Target = 150
                num_test_Shadow = 6000

            elif data_type == "Coauthor_Phys" or data_type == "Phys":
            ...
            if len(self.test_indices) > len(self.node_removes):
                ...
                ), random.sample(self.test_indices.tolist(), 270)
                self.train_indices = random.sample(self.train_indices.tolist(), 270)

            self.shadow_positive = self.posterior[self.train_indices]
            # random_nodes = np.array(random_nodes)
            ...
            # self.train_posterior = self.test_posterior
            ...
            class TargetModel(torch.nn.Module):
                def __init__(self, dataset):
                    ...
                    if model_type == "GCN":
                        ...
                        # SGC
                        self.conv1 = SGConv(
                            dataset.num_node_features, 16, K=2, cached=False
                        )
                        self.conv2 = SGConv(16, dataset.num_classes, K=2, cached=False)
            ...
            self.return_auc = auroc
            self.return_accuracy = test_accuracyInOut
    ...
    def get_results(self):
        return self.acc_pos, self.f1_pos, self.return_auc, self.return_accuracy
